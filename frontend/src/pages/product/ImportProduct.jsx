import { useNavigate } from "react-router-dom";
import Container from "../../components/styled/Container";
import { Button, Card, Form, Typography } from "antd";
import FormButtonGroup from "../../components/styled/FormButtonGroup";
import FormImportExport from "../../components/form/FormImportExport";
import { apiCaller } from "../../utils/api";
import productAPI from "../../api/requests/product";
import AntdSwal from "../../custom/AntdSwal";

const { Title } = Typography;

const ImportProduct = () => {
  const navigate = useNavigate();
  const [form] = Form.useForm();

  const handleImportProduct = (values) => {
    apiCaller(productAPI.createHistory(values), () => {
      AntdSwal.fire("Thành công", "Nhập kho thành công!", "success");
      navigate("/stock-history");
    });
  };

  return (
    <Container size="lg">
      <Title level={2} className="mt-4">
        Nhập kho
      </Title>
      <Card title="Thông tin hàng nhập" bordered={false} className="mt-8">
        <FormImportExport form={form} onFinish={handleImportProduct} type="IMPORT" />
      </Card>
      <FormButtonGroup>
        <Button onClick={() => navigate("/employees")}>Hủy</Button>
        <Button type="primary" onClick={() => form.submit()}>
          Nhập kho
        </Button>
      </FormButtonGroup>
    </Container>
  );
};

export default ImportProduct;
